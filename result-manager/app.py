...
@app.route('/api/test_result_summary', methods=['POST'])
def update_test_result_summary():
    # data : { "test_result_seq": 9 }
    data = request.get_json()

    # get Test_task_run
    test_task_runs = Test_task_run.query.filter(Test_task_run.test_result_seq == data['test_result_seq'])
    ...
    if run_count == 0:
        ...
        test_result = Test_result.query.get(data['test_result_seq'])

        if test_result:
            print(test_result)

            test_result.run_count = run_count
            test_result.ok_count = ok_count
            test_result.ng_count = ng_count
            # update
            db.session.commit()
            ...
            return_text = {
                ...
                'ok_count': ok_count,
                'ng_count': ng_count,
                'status': "Error: test_result is nothing"
            }
            # return
            return jsonify(return_text), 400
